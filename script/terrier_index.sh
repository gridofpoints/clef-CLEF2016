...
declare -a collections=("en_tipster_9799" "en_ah_mono_0002" "en_ah_mono_0306" "en_ah_mono_0707" "bg_ah_mono_0507" "de_ah_mono_0000" "de_ah_rob_mono_0106" "es_ah_mono_0102" "es_ah_rob_mono_0306" "fa_ah_mono_0809" "fi_ah_mono_0204" "fr_ah_mono_0000" "fr_ah_mono_0102" "fr_ah_rob_mono_0307" "fr_ah_mono_0404" "fr_ah_mono_0506" "hu_ah_mono_0507" "it_ah_mono_0002" "it_ah_rob_mono_0306" "nl_mono_0106" "pt_ah_mono_0404" "pt_ah_rob_mono_0506" "ru_ah_mono_0304" "cs_ah_mono_0707" "sv_ah_mono_0203");
...
# available stoplists
# Terrier standard
sl_name=stoplist.txt
sl_name2=stoplist2.txt

stoplist=$sl_path$sl_name;
stoplist2=$sl_path$sl_name2;

## declare the stoplists array
declare -a stopwords=(stopUnine stopSnow nostop);

declare -a ngrams=(3grams 4grams 5grams 6grams 7grams 8grams 9grams 10grams nograms);
...
case "$lang" in
    bg) indexDir="$lang";
		...
		declare -a stemmerNames=(wkstem agstem nolug);
    ...
    en) indexDir="$lang";
			...
			declare -a stopwords=(nostop indri lucene mild proteus smart terrier);
			...
			if [ "$collection_name" == "en_tipster_96" ]; then
				...
				ls -1 ${corpusPaths["en_tipster_96"]}/TREC_VOL4 | xargs -i cp ${corpusPaths["en_tipster_96"]}/TREC_VOL4/{} "$path"/corpus;
				ls -1 ${corpusPaths["en_tipster_96"]}/TREC_VOL2 | xargs -i cp ${corpusPaths["en_tipster_96"]}/TREC_VOL2/{} "$path"/corpus;
				...
				ls -1 ${corpusPaths["en_tipster_97"]}/TREC_VOL4 | xargs -i cp ${corpusPaths["en_tipster_97"]}/TREC_VOL4/{} "$path"/corpus;
				ls -1 ${corpusPaths["en_tipster_97"]}/TREC_VOL5 | xargs -i cp ${corpusPaths["en_tipster_97"]}/TREC_VOL5/{} "$path"/corpus;
...
create_terrier_properties()
{
	...
	if [ "$gramCond" = true ]; then
		if [ "$gram" = "${ngrams[0]}" ]; then
			printf "tokeniser.ngram=3\n" >> "$file"
		elif [ "$gram" = "${ngrams[1]}" ]; then
			printf "tokeniser.ngram=4\n" >> "$file"
		elif [ "$gram" = "${ngrams[2]}" ]; then
			printf "tokeniser.ngram=5\n" >> "$file"	
		elif [ "$gram" = "${ngrams[3]}" ]; then
			printf "tokeniser.ngram=6\n" >> "$file"
		elif [ "$gram" = "${ngrams[4]}" ]; then
			printf "tokeniser.ngram=7\n" >> "$file"	
		elif [ "$gram" = "${ngrams[5]}" ]; then
			printf "tokeniser.ngram=8\n" >> "$file"	
		elif [ "$gram" = "${ngrams[6]}" ]; then
			printf "tokeniser.ngram=9\n" >> "$file"	
		elif [ "$gram" = "${ngrams[7]}" ]; then
			printf "tokeniser.ngram=10\n" >> "$file"	
		fi
	fi
			...
			if [ "$st" == "${stemmers[0]}" ]; then
				...
				do
					if [ "$gram" == "${ngrams[0]}" ]; then
						...
						gramCond=true;
						# no stemmer
						currIndexDir=$indexDir"${stopwords[0]}"_"${ngrams[2]}";

						#the name of the property file for creating this index
						propertyName="${stopwords[0]}"_"${ngrams[2]}".properties;
						...
						gramCond=true;
						# no stemmer
						currIndexDir=$indexDir"${stopwords[0]}"_"${ngrams[3]}";

						#the name of the property file for creating this index
						propertyName="${stopwords[0]}"_"${ngrams[3]}".properties;

						mkdir $currIndexDir
						printf "Indexing TIPSTER with Stoplist: %s no Stemmer and %s" "${stopwords[0]}" "${ngrams[3]}";		
					elif [ "$gram" == "${ngrams[4]}" ]; then
						gramCond=true;
						# no stemmer
						currIndexDir=$indexDir"${stopwords[0]}"_"${ngrams[4]}";

						#the name of the property file for creating this index
						propertyName="${stopwords[0]}"_"${ngrams[4]}".properties;

						mkdir $currIndexDir
						printf "Indexing TIPSTER with Stoplist: %s no Stemmer and %s" "${stopwords[0]}" "${ngrams[4]}";
					elif [ "$gram" == "${ngrams[5]}" ]; then
						gramCond=true;
						# no stemmer
						currIndexDir=$indexDir"${stopwords[0]}"_"${ngrams[5]}";

						#the name of the property file for creating this index
						propertyName="${stopwords[0]}"_"${ngrams[5]}".properties;

						mkdir $currIndexDir
						printf "Indexing TIPSTER with Stoplist: %s no Stemmer and %s" "${stopwords[0]}" "${ngrams[5]}";
					elif [ "$gram" == "${ngrams[6]}" ]; then
						gramCond=true;
						# no stemmer
						currIndexDir=$indexDir"${stopwords[0]}"_"${ngrams[6]}";

						#the name of the property file for creating this index
						propertyName="${stopwords[0]}"_"${ngrams[6]}".properties;

						mkdir $currIndexDir
						printf "Indexing TIPSTER with Stoplist: %s no Stemmer and %s" "${stopwords[0]}" "${ngrams[6]}";	
					elif [ "$gram" == "${ngrams[7]}" ]; then
						gramCond=true;
						# no stemmer
						currIndexDir=$indexDir"${stopwords[0]}"_"${ngrams[7]}";

						#the name of the property file for creating this index
						propertyName="${stopwords[0]}"_"${ngrams[7]}".properties;

						mkdir $currIndexDir
						printf "Indexing TIPSTER with Stoplist: %s no Stemmer and %s" "${stopwords[0]}" "${ngrams[7]}";											
					elif [ "$gram" == "${ngrams[8]}" ]; then
						gramCond=false;
						# no stemmer
						...
						mkdir $currIndexDir
						printf "Indexing TIPSTER with Stoplist: %s no Stemmer and %s" "${stopwords[0]}" "${ngrams[8]}";		
					fi
			...
			if [ "$st" == "${stemmers[0]}" ]; then
				...
				do
					if [ "$gram" == "${ngrams[0]}" ]; then
						...
						gramCond=true;
						# no stemmer
						currIndexDir=$indexDir"${stopwords[1]}"_"${ngrams[2]}";

						#the name of the property file for creating this index
						propertyName="${stopwords[1]}"_"${ngrams[2]}".properties;
						...
						gramCond=true;
						# no stemmer
						currIndexDir=$indexDir"${stopwords[1]}"_"${ngrams[3]}";

						#the name of the property file for creating this index
						propertyName="${stopwords[1]}"_"${ngrams[3]}".properties;

						mkdir $currIndexDir
						printf "Indexing TIPSTER with Stoplist: %s no Stemmer and %s" "${stopwords[1]}" "${ngrams[3]}";		
					elif [ "$gram" == "${ngrams[4]}" ]; then
						gramCond=true;
						# no stemmer
						currIndexDir=$indexDir"${stopwords[1]}"_"${ngrams[4]}";

						#the name of the property file for creating this index
						propertyName="${stopwords[1]}"_"${ngrams[4]}".properties;

						mkdir $currIndexDir
						printf "Indexing TIPSTER with Stoplist: %s no Stemmer and %s" "${stopwords[1]}" "${ngrams[4]}";
					elif [ "$gram" == "${ngrams[5]}" ]; then
						gramCond=true;
						# no stemmer
						currIndexDir=$indexDir"${stopwords[1]}"_"${ngrams[5]}";

						#the name of the property file for creating this index
						propertyName="${stopwords[1]}"_"${ngrams[5]}".properties;

						mkdir $currIndexDir
						printf "Indexing TIPSTER with Stoplist: %s no Stemmer and %s" "${stopwords[1]}" "${ngrams[5]}";
					elif [ "$gram" == "${ngrams[6]}" ]; then
						gramCond=true;
						# no stemmer
						currIndexDir=$indexDir"${stopwords[1]}"_"${ngrams[6]}";

						#the name of the property file for creating this index
						propertyName="${stopwords[1]}"_"${ngrams[6]}".properties;

						mkdir $currIndexDir
						printf "Indexing TIPSTER with Stoplist: %s no Stemmer and %s" "${stopwords[1]}" "${ngrams[6]}";	
					elif [ "$gram" == "${ngrams[7]}" ]; then
						gramCond=true;
						# no stemmer
						currIndexDir=$indexDir"${stopwords[1]}"_"${ngrams[7]}";

						#the name of the property file for creating this index
						propertyName="${stopwords[1]}"_"${ngrams[7]}".properties;

						mkdir $currIndexDir
						printf "Indexing TIPSTER with Stoplist: %s no Stemmer and %s" "${stopwords[1]}" "${ngrams[7]}";											
					elif [ "$gram" == "${ngrams[8]}" ]; then
						gramCond=false;
						# no stemmer
						...
						mkdir $currIndexDir
						printf "Indexing TIPSTER with Stoplist: %s no Stemmer and %s" "${stopwords[1]}" "${ngrams[8]}";		
					fi
			...
			if [ "$st" == "${stemmers[0]}" ]; then
				...
				do
					if [ "$gram" == "${ngrams[0]}" ]; then
						...
						gramCond=true;
						# no stemmer
						currIndexDir=$indexDir"${stopwords[2]}"_"${ngrams[2]}";

						#the name of the property file for creating this index
						propertyName="${stopwords[2]}"_"${ngrams[2]}".properties;
						...
						gramCond=true;
						# no stemmer
						currIndexDir=$indexDir"${stopwords[2]}"_"${ngrams[3]}";

						#the name of the property file for creating this index
						propertyName="${stopwords[2]}"_"${ngrams[3]}".properties;

						mkdir $currIndexDir
						printf "Indexing TIPSTER with Stoplist: %s no Stemmer and %s" "${stopwords[2]}" "${ngrams[3]}";		
					elif [ "$gram" == "${ngrams[4]}" ]; then
						gramCond=true;
						# no stemmer
						currIndexDir=$indexDir"${stopwords[2]}"_"${ngrams[4]}";

						#the name of the property file for creating this index
						propertyName="${stopwords[2]}"_"${ngrams[4]}".properties;

						mkdir $currIndexDir
						printf "Indexing TIPSTER with Stoplist: %s no Stemmer and %s" "${stopwords[2]}" "${ngrams[4]}";
					elif [ "$gram" == "${ngrams[5]}" ]; then
						gramCond=true;
						# no stemmer
						currIndexDir=$indexDir"${stopwords[2]}"_"${ngrams[5]}";

						#the name of the property file for creating this index
						propertyName="${stopwords[2]}"_"${ngrams[5]}".properties;

						mkdir $currIndexDir
						printf "Indexing TIPSTER with Stoplist: %s no Stemmer and %s" "${stopwords[2]}" "${ngrams[5]}";
					elif [ "$gram" == "${ngrams[6]}" ]; then
						gramCond=true;
						# no stemmer
						currIndexDir=$indexDir"${stopwords[2]}"_"${ngrams[6]}";

						#the name of the property file for creating this index
						propertyName="${stopwords[2]}"_"${ngrams[6]}".properties;

						mkdir $currIndexDir
						printf "Indexing TIPSTER with Stoplist: %s no Stemmer and %s" "${stopwords[2]}" "${ngrams[6]}";	
					elif [ "$gram" == "${ngrams[7]}" ]; then
						gramCond=true;
						# no stemmer
						currIndexDir=$indexDir"${stopwords[2]}"_"${ngrams[7]}";

						#the name of the property file for creating this index
						propertyName="${stopwords[2]}"_"${ngrams[7]}".properties;

						mkdir $currIndexDir
						printf "Indexing TIPSTER with Stoplist: %s no Stemmer and %s" "${stopwords[2]}" "${ngrams[7]}";											
					elif [ "$gram" == "${ngrams[8]}" ]; then
						gramCond=false;
						# no stemmer
						...
						mkdir $currIndexDir
						printf "Indexing TIPSTER with Stoplist: %s no Stemmer and %s" "${stopwords[2]}" "${ngrams[8]}";		
					fi
					# execute the function which creates the properties file
					create_terrier_properties
