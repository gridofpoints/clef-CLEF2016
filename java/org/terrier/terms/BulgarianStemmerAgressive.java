 ...
 * (original taken from http://members.unine.ch/jacques.savoy/clef/index.html by
 * Ljiljana Dolamic)
 * 
 ...
 * 
 */
public class BulgarianStemmerAgressive extends StemmerTermPipeline {
	 ...
	 * constructor
	 */
	public BulgarianStemmerAgressive() {
		super();
	}
	 ...
	 * @param next
	 */
	public BulgarianStemmerAgressive(TermPipeline next) {
		super(next);
	}
	...
	public String stem(String word) {
		...
		remove_plural(sb);
		/* remove the last latter under given conditions */
		normalize(sb);
		/* remove derivational suffix */
		remove_derivational(sb);
		palatalisation(sb);
	...
	private void remove_derivational(StringBuilder word) {
		int len = word.length() - 1;
		/*-chenc ,diminutive m*/
		if (len > 7) {
			if (word.substring(len - 5, len + 1).equals("chents")) {
		...
		/*-chits,diminutive f*/
		if (len > 6) {
			if (word.substring(len - 4, len + 1).equals("chits")) {
		...
		if (len > 6) {
			/*-ichka ,diminutive f*/
			if (word.substring(len - 4, len + 1).equals("ichk")) {
			...
			/*-estvo ,der. n*/
			if (word.substring(len - 4, len + 1).equals("estv")) {
		...
		if (len > 4) {
			...
			/*-eski,diminutive n*/
			if (word.substring(len - 2, len + 1).equals("esk")) {
			...
			/*-itsa ,diminutive f*/
			if (word.substring(len - 2, len + 1).equals("its")) {
			...
			/*-ntse,diminutive n*/
			if (word.substring(len - 2, len + 1).equals("nts") || word.substring(len - 1, len + 1).equals("ets")) {
		...
		if (len > 3) {
			...
			/*-che ,diminutive m*/
			/*-ova,ovi,ovo ,poss m*/
			/*-ska,ski,sko der. f,pl,n*/
			/*-ets der. m*/
			if (word.substring(len - 1, len + 1).equals("ch") || word.substring(len - 1, len + 1).equals("ov")
					|| word.substring(len - 1, len + 1).equals("sk")) {
			...
			if (word.substring(len - 1, len + 1).equals("il") || word.substring(len - 1, len + 1).equals("ev")
					|| word.substring(len - 1, len + 1).equals("in")) {

				word.delete(len - 1, len + 1);
				// palatalisation(word);
				return;
			}
			if (word.substring(len - 1, len + 1).equals("en")) {
				if (isVowel(word.charAt(len - 2)) || ((word.charAt(len - 2) == 'h') && (word.charAt(len - 3) == 'u'))) {

					word.setCharAt(len - 1, 'j');
		...
		if (len > 3) {
			...
			if (word.substring(len - 1, len + 1).equals("ov") || word.substring(len - 1, len + 1).equals("ni")
					|| word.substring(len - 1, len + 1).equals("na") || word.substring(len - 1, len + 1).equals("no")
					|| word.substring(len - 1, len + 1).equals("ka")) {
			...
			if (word.substring(len, len + 1).equals("v") || word.substring(len, len + 1).equals("n")) {
	...
	public static void main(String[] args) {
		try {
			BulgarianStemmerAgressive stemmer = new BulgarianStemmerAgressive();
			createNewFile();
	...
	private static void createNewFile() {
		try {
			...
			osw = new OutputStreamWriter(new FileOutputStream("BgAgressive.dat"), "UTF-8");
